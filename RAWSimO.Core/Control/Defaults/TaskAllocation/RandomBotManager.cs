    ...
    public class RandomBotManager : BotManager
    {
        ...
        private bool DoExtractTaskWithPod(Bot bot, Pod pod)
        {
            ...
            foreach (var oStation in Instance.OutputStations.OrderBy(s => GetOrderValue(s, bot)))
            {
                ...
                {
                    int randIdx = Instance.Randomizer.NextInt(0, fittingRequests.Count());
                    ExtractRequest randRequest = fittingRequests[randIdx];
                    ...
                    EnqueueExtract(
                        ...
                        pod, // Keep the pod
                        //fittingRequests);
                        new List<ExtractRequest> { randRequest }); // The first requests to serve
        ...
        protected override void GetNextTask(Bot bot)
        {
            ...
            if (_extractMode[bot])
            {
                List<Pod> avaiPods = Instance.ResourceManager.UnusedPods.OrderBy(b => GetOrderValue(b, bot)).ToList();
                int cnts = avaiPods.Count();
                while (cnts != 0)
                {
                    // find a random pod
                    int randIdx = Instance.Randomizer.NextInt(0, avaiPods.Count());
                    Pod pod = avaiPods[randIdx];
